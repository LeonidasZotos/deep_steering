...
for model in qwen3-4b-base qwen3-8b-base qwen3-0.6b-chat qwen3-1.7b-chat qwen3-4b-chat qwen3-8b-chat
do
    for permutation_strategy in random reverse
    do
        for system_prompt in auth_left auth_right lib_left lib_right generic centrist
        do
            python ./answer_questionnaire.py -q political_compass -m $model -ps $permutation_strategy -per $system_prompt -b 16
        done
    done
done
